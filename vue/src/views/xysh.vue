<template>
  <div v-if="user.role != '2'" style="padding: 10px">
    <!--    功能区-->
    <div style="margin: 10px 0">
      <el-select v-model="search_term" clearable filterable placeholder="查看历史学年申请" style="width: 10%;"
                 @change="load">
        <el-option
      ...
      <el-input v-model="search" clearable placeholder="请输入学号" style="width: 10%;margin-left: 20px"
                @input="load"></el-input>
      <el-input v-model="search_name" clearable placeholder="请输入姓名" style="width: 10%;margin-left: 20px"
                @input="load"></el-input>
      <el-select v-model="search_speed" clearable filterable placeholder="当前审批状态"
                 style="width: 10%;margin-left: 20px" @change="load">
      ...
      <el-select v-model="search_type" clearable filterable placeholder="资助认定状态"
                 style="width: 10%;margin-left: 20px" @change="load">
    ...
    <el-table :cell-style="{'text-align':'center'}" :data="tableData" :header-cell-style="{'text-align':'center'}"
      ...
      <el-table-column fixed="right" label="操作" width="auto">
        <template #default="scope">
          <el-button size="small" type="primary" @click="handleEdit(scope.row)">申请查看</el-button>
          <span v-if="user.role != '2'" style="margin-left: 10px">
                        <span v-if="scope.row.speed != 'remain'">
                        <el-button v-if="scope.row.speed != 'save'" size="small" type="success"
                                   @click="savepass(scope.row.id)">通过</el-button>
                        <el-button v-if="scope.row.speed != 'save'" size="small" type="danger"
                                   @click="saverefuse(scope.row.id)">驳回</el-button>
                        </span>
                    </span>
          ...
          <span v-if="user.role === '1'">
                        <span :hidden="menuRoleSet" style="margin-left: 10px">
                            <el-popconfirm title="确认删除?" @confirm="handleDelete(scope.row.id)">
                                <template #reference>
                                    <el-button :disabled="menuRoleSet" size="small" type="danger">删除</el-button>
                                </template>
                            </el-popconfirm>
                        </span>
                    </span>
...
export default {
  ...
  methods: {
    exp() {
      ...
      window.open("/api/qgsq/export?name=" + this.search_name
          + "&username=" + this.search
          + "&college=" + this.user.college
          + "&term=" + this.search_term
          + "&type=" + this.search_type
          + "&speed=" + this.search_speed,)
    ...
    getTermInfo() {
      request.get("/term/").then(res => {
        console.log("terminfo", res)
        this.xnNow = res[res.length - 1]
        this.terminfo = res
        this.search_term = this.xnNow.pcdm
        this.load()
      })
    },
    add() {
    ...
    load() {
      request.get("/qgsq", {
        params: {
          ...
          search_speed: this.search_speed
        }
      }).then(res => {
        console.log(res)
